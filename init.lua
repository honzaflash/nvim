--[[
  ...
  TODO: Go through the whole `:Tutor` in Neovim.
...
-- Alternative for terminals with colliding keymaps or not able to send distinct keycodes
...
vim.keymap.set('n', 'gl', '<C-^>', { desc = "Switch to the [L]ast buffer" })
vim.keymap.set('n', 'ga', 'g<Tab>', { desc = "Switch to the l[A]st tab" })
...
require('lazy').setup({
  ...
  {
    'honzaflash/gruvyboxy.nvim',
    priority = 1000,
    opts = ...,
    config = function() vim.cmd.colorscheme 'gruvyboxy' end,
  },
  -- for theme develpment purposes load the local version
  -- { dir = '~/dev/nvim/gruvyboxy.nvim' },
